## Ejecutar Flujo de Integración - Consulta de Autorización de Facturación Electrónica
    ...
    "EXTRACTS": [
      {
        ...
        "configuration": {
          "connection": {
            ...
            "user": "{{user_mongo}}",
            "password": "{{pass_mongo}}",
            "repository": "QA_KFC_MXPi_Pub_Fe",
            "adapter": "Mongo"
          },
          "entities": [
            {
              "name": "Pub_Fe_Authorization",
              "properties": [],
              "filters": [
                {
                  "name": "fe_authorization_status.estado",
                  "operator": "not_in",
                  "value": ["A", "RC", "XA", "DR", "ERROR ENVIO"]
                }
              ]
            }
          ]
      ...
      },
      {
        ...
        "configuration": {
          ...
          "entities": [
            {
              "name": "api/kfc/consultarComprobante",
              "properties": [],
              "filters": []
            }
          ]
...
| `processes.EXTRACTS[].configuration.connection.server`                 | string  | Servidor de base de datos o URL del servicio         |
...
| `processes.EXTRACTS[].configuration.connection.repository`             | string  | Nombre de la base de datos o ruta del endpoint       |
...
| `processes.EXTRACTS[].configuration.entities[].name`                   | string  | Nombre de la colección o endpoint                    |
...
| `processes.EXTRACTS[].configuration.entities[].filters[].value`        | array   | Lista de valores del parámetro                       |
