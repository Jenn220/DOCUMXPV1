...
#### 500 Internal Server Error - Error en Fase de Extracción
...
#### 500 Internal Server Error - Error en Fase de Carga
...
#### Descripción de las Respuestas de Error
...
| `syncId`                     | ID de sincronización                                                 |
| `data`                       | Objeto con los resultados de cada fase                               |
| `data.EXTRACTS`              | Array con resultados de extracciones (exitosas o fallidas)           |
| `data.EXTRACTS[].code`       | Código de estado (200 = éxito, 500 = error)                          |
| `data.EXTRACTS[].status`     | Estado de la fase                                                    |
| `data.EXTRACTS[].process`    | Código del proceso                                                   |
| `data.EXTRACTS[].message`    | Mensaje descriptivo del resultado o error                            |
| `data.EXTRACTS[].dateTime`   | Fecha y hora de ejecución                                            |
| `data.TRANSFORMS`            | Array con resultados de transformaciones (vacío si no se ejecutaron) |
| `data.TRANSFORMS[].code`     | Código de estado                                                     |
| `data.TRANSFORMS[].status`   | Estado de la fase                                                    |
| `data.TRANSFORMS[].process`  | Código del proceso                                                   |
| `data.TRANSFORMS[].message`  | Mensaje descriptivo                                                  |
| `data.TRANSFORMS[].dateTime` | Fecha y hora de ejecución                                            |
| `data.LOAD`                  | Array con resultados de cargas (vacío si no se ejecutaron)           |
| `data.LOAD[].code`           | Código de estado                                                     |
| `data.LOAD[].status`         | Estado de la fase                                                    |
| `data.LOAD[].process`        | Código del proceso                                                   |
| `data.LOAD[].message`        | Mensaje descriptivo del error                                        |
| `data.LOAD[].dateTime`       | Fecha y hora del error                                               |
